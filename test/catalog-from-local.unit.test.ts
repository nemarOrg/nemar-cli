...
describe("buildCatalogRecordFromLocal", () => {
  ...
  test("publish_date prefers first_version_at when set, falls back to created_at", () => {
    expect(buildCatalogRecordFromLocal(row({ first_version_at: null }), null).publish_date).toBe(
      "2026-04-08 08:32:30",
    );
    expect(
      buildCatalogRecordFromLocal(row({ first_version_at: "2026-05-11 13:48:29" }), null)
        .publish_date,
