...
> **Correction (2026-08-23, #1145 / epic #1144 phase 1):** the "no offset; `limit ≤ 100`" claim above is stale. `count` used to be the length of a JS-filtered slice of `limit * 2` candidates, so it drifted with page size (`q=eeg&has_hed=1` read `1`/`7`/`13`/`23`/`43` at `limit=5/10/20/50/100` against a true total of `189`). Phase 1 fixed this: both retrieval legs now fetch fixed candidate windows (`SEARCH_CANDIDATE_CEILING=300`, `SEMANTIC_TOPK=100`) independent of `limit`, `count` is a dedicated exact `COUNT(*)` (`countSearchMatches`) over the same union predicate the results are drawn from, `modality`/`has_hed` filter in SQL for every tier (including exact-id) instead of being reapplied in JS, and the endpoint now accepts and clamps `offset` (`parseSearchPagination`). The envelope gained `returned`, `offset`, `limit`, `candidate_ceiling`; `limit` is still capped at 100. See the Phase 1 PR for #1145.
