...
> **Correction (2026-08-23, #1145 / epic #1144 phase 1; amended after PR review round 3):** the "no offset; `limit ≤ 100`" claim above is stale. `count` used to be the length of a JS-filtered slice of `limit * 2` candidates, so it drifted with page size (`q=eeg&has_hed=1` read `1`/`7`/`13`/`23`/`43` at `limit=5/10/20/50/100` against a true total of `189`). Phase 1 fixed this: both retrieval legs now fetch fixed candidate windows (`SEARCH_CANDIDATE_CEILING=300`, `SEMANTIC_TOPK=100`) independent of `limit`, and `count` is a dedicated exact `COUNT(*)` (`countSearchMatches`) over the full query+filter predicate. That `COUNT(*)` is deliberately *unbounded* -- independent of the candidate windows `results` is drawn from -- which is what lets it reach the true total even past those windows: `count` can legitimately exceed `candidate_ceiling` (an envelope field reporting the actual candidate-pool size for that response), and paging past it returns an empty page while `count` stays truthful (the envelope's derived `truncated` boolean flags this). `modality`/`has_hed` filter in SQL for every tier (including exact-id) instead of being reapplied in JS, and the endpoint now accepts and clamps `offset` (`parseSearchPagination`). The envelope gained `returned`, `offset`, `limit`, `candidate_ceiling`, `truncated`, and an optional `warning` (set when a `countSearchMatches` failure falls back to a page-derived lower bound rather than 500ing); `limit` is still capped at 100. See the Phase 1 PR for #1145.
