...
- **The `biosigio` pin is no longer single-sourced.** `nemarDatasets/.github` still needs it for `patch_duration.py` (the Tier-2 duration backfill inside `generate-records.yml`), so that repo gets its own `scripts/requirements.txt` pinning `biosigio[meg,mef3,hdf5]>=1.2.4` — the importers without the `[zarr]` store-writer extra it never used. Two pins now exist and must be bumped together. This is a genuine regression against the old arrangement and is called out in both files' headers.
