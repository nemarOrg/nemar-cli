 ...
 */
sandboxRoutes.post("/complete", zValidator("json", completeSchema), async (c) => {
  const user = c.get("user");
  const db = c.env.DB;
  const { dataset_id } = c.req.valid("json");
  ...
  try {
    ...
    console.error("Error completing sandbox:", error);
    return c.json({ error: "Failed to complete sandbox training" }, 500);
  }
});
...
sandboxRoutes.post("/reset", async (c) => {
  ...
  try {
    ...
    return c.json({ error: "Failed to reset sandbox status" }, 500);
  }
});
