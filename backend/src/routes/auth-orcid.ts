...
async function linkIdentity(
  ...
  if (decision.setUsersOrcid) {
    await env.DB.prepare("UPDATE users SET orcid = ?, orcid_verified = ? WHERE id = ?")
      .bind(decision.setUsersOrcid, decision.orcidVerified, userId)
      .run();
  } else {
    await env.DB.prepare("UPDATE users SET orcid_verified = ? WHERE id = ?")
      .bind(decision.orcidVerified, userId)
      .run();
  }
}
...
authOrcidRoutes.post("/orcid/unlink", webSessionMiddleware, async (c) => {
  ...
  await c.env.DB.prepare("DELETE FROM oauth_identities WHERE user_id = ? AND provider = 'orcid'")
    .bind(webUser.id)
    .run();
  await c.env.DB.prepare("UPDATE users SET orcid_verified = 0 WHERE id = ?").bind(webUser.id).run();
