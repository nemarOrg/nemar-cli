...
/**
 * POST /admin/datasets/:id/enrich - Run the full LLM enrichment pipeline server-side
 *
 * Runs the same pipeline as the webhook /llm-enrich endpoint but authenticated
 * via admin API key (no webhook token needed). Uses the backend's OPENROUTER_API_KEY.
 * Supports ?force=true to re-enrich even if already validated.
 */
  ...
  try {
    ...
    try {
      ...
      if (s3Stats.totalSize > totalSize) {
        ...
        fileCount = s3Stats.objectCount + files.length;
      }
    } catch {
