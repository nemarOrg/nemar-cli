...
the pattern `gh auth login` uses, because a terminal cannot receive an OAuth redirect.
Five routes: `POST /auth/device/start` (CLI mints a device code + user code),
`POST /auth/device/token` (CLI polls for the key),
`GET /auth/device/lookup`, `POST /auth/device/confirm`, and `POST /auth/device/deny`
...
confirms at `/confirm`** (ADR 0047): confirm records `user_id` and `status='confirmed'`
only, so no plaintext key is ever at rest between the two steps.
Every key is a named row for one machine (`tokens.name`), and a new sign-in never
revokes another machine's key.
`POST /auth/device/token` is the one route in this family deliberately OUTSIDE the
strict `AUTH_PATHS` bucket — the CLI polls it every 5 seconds, which would trip a
10/min floor on the third poll — and rides the generic bucket plus its own per-row
5-second floor (`slow_down`) instead.
Every timestamp this flow writes or compares is SQL-side (`datetime('now', ...)`,
`julianday`), never a JS `toISOString()` value, because the two compare unequally
on the same day and silently break expiry.
